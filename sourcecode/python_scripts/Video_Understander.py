...
class VideoUnderstander(object):
    ...
    def __init__(self,creation_data):
        ...
        if "raw_video_loc" not in creation_data and "containgframenumber_video_loc" not in creation_data and "raw_frame_loc" not in creation_data and "containgframenumber_frame_loc" not in creation_data:
            raise Exception("Not a Single Location Provided")


        if "raw_video_loc" in creation_data:
    ...
    def __extract_Frames_From_Given_State__(self,start,end,project_name,state):
        ...
        if state == VideoTypes.RAW:
            ...
            try:
                ...
                video = cv2.VideoCapture(self.raw_video_location)
                
                
            except Exception as E:
                
                
                raise Exception("Raw Video Location is INVALID or Not Present")
        ...
        if state == VideoTypes.CONTAININGFRAMENUMBER:
            ...
            try:
                ...
                video = cv2.VideoCapture(self.containgframenumber_video_location)
                
                
            except Exception as E:
                
                
                raise Exception("ContaingFrameNumber Video Location is INVALID or Not Present")
